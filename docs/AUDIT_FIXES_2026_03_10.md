...
| M8 No sentinel for empty title-break search | Fixed | `src/services/pg_title_break_service.py`, `src/services/pg_pipeline_controller.py` | Empty ORI deed searches now insert expiring `SEARCH_NO_RESULT` sentinels, complete/gapless chains are excluded from scope, and sentinel-writing runs report degraded instead of `noop`. |

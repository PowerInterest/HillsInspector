...
class _FakeSession:
    ...
    def execute(self, *_args: Any, **_kwargs: Any) -> None:
        raise AssertionError("empty entity loads should not attempt row inserts")


def test_load_sunbiz_entity_marks_zero_row_files_failed(
    monkeypatch: Any,
    tmp_path: Any,
) -> None:
    empty_zip = tmp_path / "cor" / "cordata.zip"
    ...
    monkeypatch.setattr(pg_loader, "_upsert_ingest_file", lambda **_kwargs: 7)
    monkeypatch.setattr(
        pg_loader,
        "_iter_text_records",
    ...
    with pytest.raises(RuntimeError, match="No entity records parsed"):
        pg_loader.load_sunbiz_entity(
            dsn="postgresql://db",
            root=tmp_path,
            pattern=None,
            limit_files=None,
    ...
    assert marked == [
        {
            ...
            "status": "failed",
            "row_count": None,
            "error_message": "No entity records parsed from cor/cordata.zip; refusing to mark empty load as current",
        }
    ]

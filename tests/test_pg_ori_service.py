...
def test_find_targeted_recovery_targets_sql_does_not_require_strap(
    monkeypatch: Any,
) -> None:
    ...
    targets = service._find_targeted_recovery_targets(  # noqa: SLF001
        foreclosure_ids=[7],
        limit=None,
    )
    ...
    assert "f.strap is not null" not in sql_text
    assert targets[0]["foreclosure_id"] == 7
    assert targets[0]["strap"] is None


def test_run_targeted_recovery_forces_satisfaction_relink(monkeypatch: Any) -> None:
    service = _build_service(monkeypatch)
    captured_targets: list[dict[str, Any]] = []

    monkeypatch.setattr(
        service,
        "_find_targeted_recovery_targets",
        lambda **_kwargs: [
            {
                "foreclosure_id": 9,
                "case_number": "292025CA000009A001HC",
                "strap": "S9",
                "folio": "F9",
            }
        ],
    ...
    def fake_process_target(target: dict[str, Any], *, persist: bool) -> dict[str, Any]:
        ...
        return {
            ...
            "deed_count": 0,
            "clerk_case_count": 0,
            "instruments": ["2025000001"],
        }

    monkeypatch.setattr(service, "_process_target", fake_process_target)

    result = service.run_targeted_recovery(foreclosure_ids=[9])

    assert captured_targets[0]["force_satisfaction_relink"] is True
    assert result["targets_requested"] == 1
    assert result["encumbrances_saved"] == 1
    assert result["satisfactions_linked"] == 1
    assert result["per_target"][0]["foreclosure_id"] == 9

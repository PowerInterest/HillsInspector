...
def run(context: RunContext) -> StepResult:
    with timed_step(STEP_NAME) as elapsed_ms:
        cases = fetch_step_cases(context, "step_permits_checked")
        if not cases:
        ...
        async def _run(orchestrator):
            ...
            for row in cases:
                ...
                if not case_number or not parcel_id or address == "Unknown":
                    ...
                    continue
                attempted += 1
                await orchestrator._run_permit_scraper(case_number, parcel_id, address)
        ...
        summary = summarize_step_outcomes(
            ...
            step_column="step_permits_checked",
            error_step=7,
        )

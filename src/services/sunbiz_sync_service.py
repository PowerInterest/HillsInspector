...
Usage examples:
  ...
  uv run python sunbiz/sync.py sync --mode quarterly --dataset-profile entity-quarterly
...
class SunbizMirror:
    ...
    def _listdir_attr_safe(
        ...
        try:
            ...
            logger.warning(f"SFTP listdir_attr failed for {remote_dir}: {exc}")
            return None
    ...
    def _stream_download_file(
        ...
        """Download with streamed reads to reduce server-side SFTP pressure."""
        with sftp.open(remote_path, "rb") as remote_fp:
            with local_tmp_path.open("wb") as local_fp:
                while True:
                    chunk = remote_fp.read(chunk_size)
                    if not chunk:
                        break
    ...
    @staticmethod
    def _matches_dataset_profile(remote_path: str, profile: str | None) -> bool:
        """Apply high-level dataset selection filters."""
        if profile in (None, "all"):
            return True

        normalized = remote_path.lower()
        if profile == "entity-quarterly":
            return bool(
                re.search(
                    (
    ...
    def sync(
        ...
        try:
            ...
            pre_limit = (
                ...
                and include is None
                and exclude is None
                and dataset_profile in (None, "all")
            ...
            files = [
                f
                for f in files
                if self._matches_dataset_profile(f.path, dataset_profile)
            ]
            if max_files is not None:
            ...
            for item in files:
                ...
                for attempt in range(1, max_attempts + 1):
                    try:
                        if tmp_path.exists():
                            tmp_path.unlink()
                        self._stream_download_file(sftp, item.path, tmp_path)
                        break
                        ...
                        _print(
                            f"Retrying {item.path} after SFTP error "
                        ...
                        try:
                            ...
                            _print(f"Warning: failed to close SFTP session cleanly: {close_exc}")
                        time.sleep(delay)
                        transport, sftp = self._connect()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_common_args(p: argparse.ArgumentParser) -> None:
        ...
        p.add_argument(
            "--dataset-profile",
            choices=("all", "entity-quarterly"),
            default="all",
            help=(
                "High-level dataset selection. 'entity-quarterly' pulls quarterly "
                "business-entity datasets (Cor + Gen + Non-Profit)."
            ),
        )
        p.add_argument(
            "--remote-dir",

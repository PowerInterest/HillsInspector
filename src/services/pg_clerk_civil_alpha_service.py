"""PostgreSQL-backed civil alpha index bulk service.

Downloads and loads Hillsborough County Clerk civil alphabetical index data
(Circuit + County) from https://publicrec.hillsclerk.com/Civil/alpha_index/.
...
Orchestrator usage (fire-and-forget)::
    ...
    service = PgClerkCivilAlphaService()
    if service.available:
        stats = service.update()
        logger.info(f"Clerk civil alpha update: {stats}")
...
class PgClerkCivilAlphaService:
    """Standalone service: sync + load Hillsborough Clerk civil alpha index.
    ...
    def __init__(self, dsn: str | None = None, download_dir: Path | None = None):
        ...
        self._engine = None
        self._dsn: str = ""
        self.download_dir = download_dir or DEFAULT_CIVIL_ALPHA_DIR
        ...
        try:
            ...
            logger.info("PgClerkCivilAlphaService: PostgreSQL connected")
        except SQLAlchemyError as exc:
            logger.error(
                f"PgClerkCivilAlphaService: PostgreSQL connection failed: {exc}\n"
                f"  DSN: {self._dsn!r}\n"
                f"  Traceback: {traceback.format_exc()}"
            )
        except Exception as exc:
            logger.error(
                f"PgClerkCivilAlphaService: unexpected init error: {exc}\n"
                f"  Traceback: {traceback.format_exc()}"
            )
    ...
    def update(self, force_download: bool = False) -> dict[str, Any]:
        """Download new civil alpha index files and load into PostgreSQL.
        ...
        if not self._available:
            ...
            logger.error(f"PgClerkCivilAlphaService.update: {result['error']}")
            return result

        # Import loader functions (deferred to avoid circular imports)
        try:
            from src.services.pg_loader_clerk import (
                download_clerk_civil_alpha_index,
                init_db,
                load_civil_alpha_index,
            )
        except ImportError as exc:
            result["error"] = f"Failed to import pg_loader_clerk: {exc}"
            logger.error(
                f"PgClerkCivilAlphaService.update: {result['error']}\n"
                f"  Traceback: {traceback.format_exc()}"
            )
            return result
        ...
        logger.info(
            f"PgClerkCivilAlphaService.update: starting "
        ...
        try:
            init_db(self._dsn)
            logger.info("PgClerkCivilAlphaService: tables initialized")
        except Exception as exc:
        ...
        try:
            dl_stats = download_clerk_civil_alpha_index(
                output_dir=self.download_dir,
                force=force_download,
            )
            result["download"] = dl_stats
            logger.info(
                f"PgClerkCivilAlphaService download: "
            ...
            if int(dl_stats.get("errors", 0) or 0) > 0:
                logger.warning(
                    f"PgClerkCivilAlphaService: {dl_stats['errors']} download errors "
                    f"(continuing with available files)"
                )
        except Exception as exc:
            result["error"] = f"Download failed: {exc}"
            logger.error(
                f"PgClerkCivilAlphaService.update: download phase failed: {exc}\n"
                f"  Traceback: {traceback.format_exc()}"
            )
            return result

        # Step 3: Load downloaded files into PostgreSQL (normalised tables)
        try:
            logger.info("PgClerkCivilAlphaService: loading civil alpha index ...")
            load_stats = load_civil_alpha_index(
                dsn=self._dsn,
                root=self.download_dir,
            )
            result["load"] = load_stats
            logger.info(
                f"PgClerkCivilAlphaService load: "
                f"loaded={load_stats.get('files_loaded', 0)}, "
                f"cases={load_stats.get('cases_upserted', 0)}, "
                f"parties={load_stats.get('parties_upserted', 0)}"
            )
        except SQLAlchemyError as exc:
            result["error"] = f"PG write failed during load: {exc}"
            logger.error(
                f"PgClerkCivilAlphaService: PG write failed during load: {exc}\n"
            ...
            logger.error(
                f"PgClerkCivilAlphaService: load failed: {exc}\n"
                f"  Traceback: {traceback.format_exc()}"
            )
            return result
        ...
        if result["success"]:
            logger.success(
                f"PgClerkCivilAlphaService.update completed in {elapsed:.1f}s: "
                f"downloaded={result['download'].get('downloaded', '?')}, "
                f"loaded={result['load'].get('files_loaded', '?')}, "
                f"cases={result['load'].get('cases_upserted', '?')}, "
                f"parties={result['load'].get('parties_upserted', '?')}"
            )
        else:
            logger.warning(
                f"PgClerkCivilAlphaService.update completed with errors in {elapsed:.1f}s"
            )

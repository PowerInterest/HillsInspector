...
_GENERIC_PARTY_SUBSTRINGS = (
    "UNKNOWN PARTIES CLAIMING",
    "UNKNOWN PARTY CLAIMING",
    "UNKNOWN PARTIES MAY CLAIM",
    "UNKNOWN PARTY MAY CLAIM",
...
_GENERIC_PARTY_SUFFIX_RE = re.compile(
    r"^(?P<prefix>.+?)\s*(?:;|,)?\s+AND\s+(?:THE\s+)?UNKNOWN\b.*$",
    re.IGNORECASE,
)
_GENERIC_PARTY_COMMA_SUFFIX_RE = re.compile(
    r"^(?P<prefix>.+?)\s*,\s*(?:THE\s+)?UNKNOWN\b.*$",
    re.IGNORECASE,
)
_PARTY_TYPE_SPLIT_RE = re.compile(r"[|/,;]+")

_NON_LIEN_DEFENDANT_TYPES = frozenset({
    "borrower",
    "co_borrower",
    "spouse",
    "tenant",
...
_HIGH_RISK_PARTY_TYPES = frozenset({
    "second_mortgage_holder",
    "junior_lienholder",
    "junior_mortgage_holder",
    "judgment_creditor",
    "hoa",
    "condo_association",
    "irs",
...
def _is_generic_party(name: str) -> bool:
    ...
    if not normed or normed in _GENERIC_PARTY_NAMES:
        return True
    return any(marker in normed for marker in _GENERIC_PARTY_SUBSTRINGS)


def _strip_generic_party_suffix(name: str) -> str:
    """Drop trailing unknown-party boilerplate while keeping named prefixes."""
    value = str(name or "").strip()
    if not value:
        return ""

    for pattern in (_GENERIC_PARTY_SUFFIX_RE, _GENERIC_PARTY_COMMA_SUFFIX_RE):
        match = pattern.match(value)
        if not match:
            continue
        prefix = match.group("prefix").strip(" ;,")
        if prefix and not _is_generic_party(prefix):
            return prefix
    return value


def _clean_party_name(name: str) -> str:
    """Normalize party names for audit matching and suppress boilerplate."""
    value = str(name or "").strip()
    if not value:
        return ""
    value = _strip_generic_party_suffix(value)
    if _is_generic_party(value):
        return ""
    return value


def _party_type_tags(raw: str | None) -> set[str]:
    """Split prompt-specific composite party_type strings into comparable tags."""
    value = str(raw or "").strip().lower()
    if not value:
        return {"unknown"}
    tags = {
        tag.strip()
        for tag in _PARTY_TYPE_SPLIT_RE.split(value)
        if tag.strip()
    }
...
def _legal_tokens(legal: str) -> frozenset[str]:
    ...
    return frozenset(t for t in tokens if len(t) > 1 or t.isdigit())


def _iter_judgment_defendants(judgment_data: dict[str, Any]) -> list[dict[str, Any]]:
    """Normalize judgment defendant payloads across prompt/schema variants."""
    raw_defendants = (
        judgment_data.get("defendants")
        or judgment_data.get("defendants_list")
        or judgment_data.get("joined_parties")
        or []
    )
    if not raw_defendants:
        legacy = judgment_data.get("defendant")
        if legacy:
            raw_defendants = [legacy]

    if isinstance(raw_defendants, dict | str):
        raw_items: list[Any] = [raw_defendants]
    elif isinstance(raw_defendants, Sequence):
        raw_items = list(raw_defendants)
    else:
        raw_items = []
    ...
    for item in raw_items:
        if isinstance(item, dict):
            name = _clean_party_name(
                str(item.get("name") or item.get("party_name") or item.get("defendant") or "")
            )
            if not name:
                continue
            defendants.append({
                "name": name,
                "party_type": str(item.get("party_type") or "unknown").strip().lower() or "unknown",
                "is_federal_entity": bool(item.get("is_federal_entity", False)),
            })
            continue

        name = _clean_party_name(str(item))
        if not name:
            continue
        defendants.append({
            "name": name,
...
def _normalize_instrument_ref(raw: str | None) -> str:
    """Normalize instrument numbers for equality checks."""
    return re.sub(r"[^A-Z0-9]", "", str(raw or "").upper())


def _normalize_record_ref(raw: str | None) -> str:
    """Normalize book/page values without changing semantic identity."""
...
def _split_party_field(raw: str) -> list[str]:
    """Split flat party strings while preserving trustee-style continuations."""
    value = raw.strip()
    ...
    if len(pieces) <= 1:
        return [value]
    if any(
        piece.upper().startswith(_CONTINUATION_NAME_PREFIXES)
...
def _party_names_from_json(raw: Any) -> list[str]:
    """Extract party names from ORI structured party JSON when present."""
    payload = raw
...
def _party_names_from_row(row: dict[str, Any], side: str) -> list[str]:
    """Return party names for one row/side, preferring structured JSON."""
    json_key = "parties_one_json" if side == "party1" else "parties_two_json"
    structured_names = _party_names_from_json(row.get(json_key))
    if structured_names:
        return structured_names
    return _split_party_field(str(row.get(side) or ""))
...
def extract_judgment_joined_party_gap(
    ...
    property.
    """
    defendants = _iter_judgment_defendants(judgment_data)
    if not defendants:
        return []
    ...
    for d in defendants:
        ...
        if not name:
            continue
        party_type = d.get("party_type", "unknown")
        party_type_tags = _party_type_tags(party_type)
        # Skip borrower/co_borrower/spouse/tenant -- they are usually named
        # defendants but are not separate lienholders.
        if party_type_tags and party_type_tags.issubset(_NON_LIEN_DEFENDANT_TYPES):
            continue
        ...
        if not matched:
            signals.append(
                AuditSignal(
                    ...
                    severity="high" if party_type_tags & _HIGH_RISK_PARTY_TYPES else "medium",
                    detail={
                        "party_name": name,
                        "party_type": party_type,
                        "is_federal": d.get("is_federal_entity", False),
                    },
                )
            )
    return signals
...
def extract_judgment_instrument_gap(
    ...
    for ei in encumbrance_instruments:
        inst = _normalize_instrument_ref(ei.get("instrument_number"))
        if inst:
            enc_instruments.add(inst)
        book = _normalize_record_ref(ei.get("book"))
        page = _normalize_record_ref(ei.get("page"))
        if book and page:
    ...
    for ref_key, label in [
        ...
        j_inst_raw = str(ref.get("instrument_number") or "").strip()
        j_book_raw = str(ref.get("recording_book") or ref.get("book") or "").strip()
        j_page_raw = str(ref.get("recording_page") or ref.get("page") or "").strip()
        j_inst = _normalize_instrument_ref(j_inst_raw)
        j_book = _normalize_record_ref(j_book_raw)
        j_page = _normalize_record_ref(j_page_raw)
            ...
            signals.append(
                AuditSignal(
                    ...
                    detail={
                        ...
                        "instrument_number": j_inst_raw or None,
                        "book": j_book_raw or None,
                        "page": j_page_raw or None,
                    },
                )
            )
    return signals
...
def extract_lp_to_judgment_party_expansion(
    ...
    return [
        AuditSignal(
            ...
            severity="high"
            if any(
                _party_type_tags(p["party_type"]) & _HIGH_RISK_PARTY_TYPES
...
def extract_lp_to_judgment_property_change(
    ...
    if j_legal and lp_legal:
        legal_match, confidence, reason = legal_descriptions_match(lp_legal, j_legal)
        ...
        if jt and lt:
            ...
            explicit_mismatch = reason.startswith((
                "Lot mismatch",
                "Block mismatch",
                "Unit mismatch",
                "Subdivision mismatch",
            ))
            if not legal_match and (explicit_mismatch or jaccard < 0.5):
                changes.append({
...
class AuditSignalExtractor:
    ...
    def _managed_conn(self, conn: Any | None = None):
        if conn is not None:
            yield conn
            return
    ...
    def extract_all_signals(
        ...
        *,
        ...
        with self._managed_conn(conn) as active_conn:
            ...
            for fid in fids:
                all_signals.extend(self.extract_signals_for(fid, conn=active_conn))
            return all_signals
    ...
    def extract_signals_for(
        ...
        *,
        ...
        with self._managed_conn(conn) as active_conn:
            fc = self._load_foreclosure(active_conn, foreclosure_id)
            if not fc:
                return []
            ...
            enc_rows = self._load_encumbrances(active_conn, strap, case_number_raw, case_number_norm)
        ...
        lp_address = fc.get("property_address")
        signals.extend(
    ...
    def _load_foreclosure(self, conn: Any, fid: int) -> dict[str, Any] | None:
        ...
        return {
            ...
            "filing_date": row[6],
            "judgment_date": row[7],
            "property_address": row[8] if len(row) > 8 else None,
        }
    ...
    def _load_encumbrances(
        ...
        rows = conn.execute(
            ...
            SELECT oe.id, oe.encumbrance_type, oe.party1, oe.party2,
                   ...
                   oe.parties_one_json, oe.parties_two_json
        ...
        return [
            {
                ...
                "parties_one_json": r[11] if len(r) > 11 else None,
                "parties_two_json": r[12] if len(r) > 12 else None,
            }
            for r in rows
        ]
    ...
    def _resolve_lp_plaintiff(
        ...
        if lp_row:
            names = _party_names_from_row(lp_row, "party1")
            if names:
                return names[0]
    ...
    def _resolve_lp_parties(
        ...
        if lp_row:
            for key in ("party1", "party2"):
                parties.extend(_party_names_from_row(lp_row, key))
        ...
        for cn in candidates:
            try:
                ...
                with contextlib.suppress(Exception):
                    conn.rollback()
                logger.debug("Failed to resolve clerk plaintiff for {}: {}", cn, exc)
                return None
            if row and row[0]:
                return row[0].strip()
        return None
    ...
    def _clerk_all_parties(
        ...
        for cn in candidates:
            try:
                ...
                with contextlib.suppress(Exception):
                    conn.rollback()
                logger.debug("Failed to resolve clerk parties for {}: {}", cn, exc)
                return []
            if rows:
    ...
    def _collect_encumbrance_parties(
        ...
        for row in enc_rows:
            for key in ("party1", "party2"):
                parties.extend(_party_names_from_row(row, key))
        return parties
    ...
    def _count_lifecycle_encumbrances(
        ...
        for row in enc_rows:
            ...
            is_lifecycle = enc_type == "assignment" or (
                enc_type == "other"
